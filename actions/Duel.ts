...
export function createDuel(random: boolean) {
    return (dispatch, getState: () => AppState) => {
        ...
        createDuelId(getState().me.player, random)
            .then(createdDuel => dispatch({type: ADD_PENDING_DUEL, duel: createdDuel}));
    }
}
...
export function deleteAndForgetPendingDuel(duel: PendingDuel) {
    return (dispatch, getState: () => AppState) => {
        ...
        deletePendingDuel(getState().me.player, duel.gameId)
            .then(() => dispatch({type: FORGET_PENDING_DUEL, duel}));
    }
}

export type DuelAction = CreateDuel | AddPendingDuel | PopDuel | ShowDuel | DeletePendingDuel | ForgetPendingDuel;
